	...
	if (available) {
		...
		else
		{
			...
			for (UINT i = 0; i < coEventsResult.size(); i++)
			{
				...
				if (dynamic_cast<CFirePot *>(e->obj)) // if e->obj is firepot 
				{
		...
		//check with camera position

		if (x > Camera::GetInstance()->GetCamX() + SCREEN_WIDTH || x < Camera::GetInstance()->GetCamY()
			|| y > Camera::GetInstance()->GetCamY() + SCREEN_HEIGHT)
		{
			this->isVanish = true;
			this->isFlying = false;
		}
